...
We tested the established model to predict medal counts for the 2028 Los Angeles Olympics. The resulting medal table is as follows:
...
%! TODO: WHICH COUNTRIES IMPROVED OR DECLINED?
...
To predict the number of countries winning their first medal, we follow these steps:

\begin{enumerate}
    \item \textbf{Data Collection}: Gather historical data on countries that have won their first Olympic medals in past Games.
    \item \textbf{Model Training}: Train a regression model using the selected features and historical data to predict the likelihood of a country winning its first medal.
    \item \textbf{Prediction}: Apply the trained model to the data for the 2028 Los Angeles Olympics to predict the number of countries that will win their first medal.
\end{enumerate}
...
\paragraph{Step 3}
For each sport $S_k$, we compute its global importance $W_k = \sum_{j \in S_k} W_j$.It is completed through aggregating the global importance values $W_j$ of all events $E_j$ under the sport, providing an overall measure of the importance of each sport for all countries collectively.
