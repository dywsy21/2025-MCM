...
\begin{abstract}
    \par 
    This study develops a robust framework for predicting medal counts at the 2028 Los Angeles Summer Olympics, grounded in an extensive analysis of historical data. The research encompasses three key tasks: forecasting medal distribution, evaluating the influence of elite coaching, and uncovering novel insights into Olympic medal dynamics.

    \textbf{Task 1} focuses on medal prediction through a tiered modeling approach. Using \textbf{K-means clustering}, we classified countries into five tiers based on historical performance, tailoring prediction models to the unique dynamics of each group. Regression techniques, including LSBoost, XGBoost, and random forests, as well as optimizing algorithms, including SSA \cite{8}, DBO \cite{9}, SCA \cite{10}, SA \cite{11}, PSO \cite{12}, were implemented with the TOPSIS method used to identify the most effective model and algorithm. Our framework predicts the emergence of new medal-winning nations and assesses the impact of host country advantages. Additionally, an analysis of the relationship between sports' global importance and medal distribution, using \textbf{Spearman's rank correlation coefficient}, confirmed the substantial benefits host countries derive, such as increased medal counts and event-specific advantages.
    
    \textbf{Task 2} explores the "Great Coach Effect" wherein elite coaches significantly influence national Olympic success. By analyzing coaching transfers and performance trends, we quantified the impact of high-caliber coaching on medal counts. Regression analysis revealed that this effect is particularly pronounced in lower-tier countries, where elite coaching can lead to substantial improvements. Based on these findings, we recommend prioritizing the recruitment of top-tier coaches in countries with the potential for notable performance gains.
    
    \textbf{Task 3} offers deeper insights into Olympic performance patterns. By examining the correlation between event participation and medal totals, we demonstrated that broader participation across multiple events correlates strongly with higher medal counts. Additionally, our analysis identified the outsized importance of specific sports, such as athletics, which consistently dominate global medal rankings. Host country effects were further scrutinized, revealing that the introduction of new events and significant increases in host medal counts contribute to an uneven competitive landscape. To address this, we propose regulatory measures to mitigate host advantages and promote fairness in future Games.
    
    This study integrates statistical modeling, machine learning, and in-depth analysis of external factors, such as elite coaching and host country dynamics, to deliver a comprehensive framework for predicting Olympic outcomes. Beyond providing accurate medal forecasts, our findings offer actionable strategies for nations to enhance their Olympic performance, fostering a more equitable and competitive global sporting environment.
...
Here are the notations and definitions used in our model and the solution of the tasks:
\begin{table}[h!]
    \centering
    \begin{tabular}{ccc}
        \toprule
...
Firstly, we calculated historical data on countries that have won their first Olympic medals in past Games, which is done in Data Preprocessing. The visualization of this dataset is shown in \textit{Figure~\ref{fig:cumulative_medal_winning_countries}}.
...
\paragraph{Step 4}
The sports are then ranked based on their global importance values $I_k$. A visualization of the rankings illustrates that athletics, boxing, and wrestling are the top three most important sports globally, where athletics has the abosolute highest(more than twice of the second place) importance value. Result is shown in \textit{Figure~\ref{fig:sport_importance}}.
