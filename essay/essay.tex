...
A key challenge we encountered was the potential for double-counting medals in team events, as each team member is associated with the same medal in the dataset. To address this, we developed a Python script to compile a comprehensive list of team events and added a column titled "Team_size", creating a new dataset referred to as "Events.csv". This dataset serves as a reference dictionary for team sizes. Additionally, we modified the "summerOly_athletes.csv" file by incorporating new columns for "Gold", "Silver", and "Bronze" medals, marking medal counts using conditional statements in Excel formulas. This step converted the "Medal" string into numerical values, enhancing compatibility with machine learning models.

Furthermore, we aimed to incorporate the host country as a training feature by matching it to competing country, while the dataset only provided host city. Recognizing the National Olympic Committee (NOC) code as a standardized and uniform identifier for countries, we created a dataset named "City_NOC.csv" to serve as a dictionary containing all Olympic host cities and their corresponding NOC codes. Using this dictionary, we developed a Python script to add a new column, "host_NOC", to the dataset while removing the original "City" column. Additionally, we excluded all data from the 1906 Olympics, as it was an informal event and disrupted the four-year cycle traditionally followed by the Olympic Games.
...
To refine the dataset further, we addressed an issue where substitutes were included in the list of athletes, rendering team size unsuitable for normalizing medal counts. To resolve this, we removed columns such as Name, Sex, Team, and Event, which were irrelevant to the country- and sport-based approach. We then eliminated duplicate rows to create a cleaned dataset, "summerOly_medal_norepeat.csv", which contained all medals without repetition or omission.
...
Features are selected and optimized using metaheuristic algorithms to determine the most effective combinations. Like the image showed, this process involves transforming the feature selection problem into a binary optimization problem, where:
\begin{itemize}
...
\subsubsection{Uncertainty Quantification and Prediction Intervals}
...
\end{figure}
\subsection{Projection for the 2028 Los Angeles Olympics}

Using the optimized models for each tier, we project the medal table for the 2028 Summer Olympics. The projections include:
\begin{itemize}
    \item Predicted Gold and total medal counts for each country.
    \item Prediction intervals representing the uncertainty in the projections.
    \item Insights into potential improvements or declines in performance compared to 2024.
\end{itemize}

\subsubsection{Identifying Trends and Insights}

\begin{itemize}
    \item \textbf{Countries Likely to Improve:} Projections indicate that countries with increasing trends in medal counts over recent Olympics and consistent improvements in performance metrics are likely to perform better.
    \item \textbf{Countries Likely to Decline:} Nations showing a decline in recent performance or with reduced investments in sports infrastructure may see a drop in medal counts.
\end{itemize}
