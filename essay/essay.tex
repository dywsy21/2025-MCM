...
\begin{abstract}
    ...
    \textbf{Task 1} focuses on medal prediction through a \textbf{tier-based modeling approach}. Using \textbf{K-means clustering}, we classified countries into five tiers based on historical performance, tailoring prediction models to the unique dynamics of each group. With \textbf{metaheuristic} algorithm, we optimized feature selection process. \textbf{Regression techniques}, including LSBoost, XGBoost, and random forests, as well as optimizing algorithms, including SSA \cite{8}, DBO \cite{9}, SCA \cite{10} were implemented with the \textbf{TOPSIS} method used to identify the most effective model and algorithm. We also introduced \textbf{K-Fold algorithm} to split datasets into training, validating and testing sets. Additionally, an analysis of the relationship between sports' global importance and medal distribution, using \textbf{Spearman's rank correlation coefficient}, confirmed the substantial benefits host countries derive, such as increased medal counts and event-specific advantages.
...
\begin{keywords}
    Olympics Medal Precition; Regression Models; K-Fold; Optimizing Algorithm; Metaheuristic Algorithm
\end{keywords}
