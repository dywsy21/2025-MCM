...
\begin{enumerate}
    ...
    \item Separates different countries into multiple clusters based on their historical medal counts (in which their comprehensive national power is also embodied \cite{2}), allowing for more accurate predictions for countries with similar overall strengths.
...
Here are the assumptions that underpin our model and the justifications for each:
...
First of all, we negated the method of making prediction based on the historical data of total medals of countries, which is logically unreasonable and too crude. As the most concrete data offered is all individual Olympic competitors with sport, event and result, it is a quite natural thought to predict the medal counts based on every individual athletes participating in certain event.
...
The first problem that came to us is that team events may result in repeated count of medals for each member in the prediction. So we used a python script to make a list of events and added a column of "Team\_size" to fix the problem, naming it "Events.csv", which will be used as a dictionary of team size. Then we added "Gold", "Silver", "Bronze" columns to summerOly_athletes.csv, and marked medal counts using Excel formula with conditional statements, converting the string "Medal" into numbers which fits model training. After that, we used a python script to divide the raw medal count with team size to normalize the data.
...
\begin{thebibliography}{99}
    ...
    \bibitem{7} Radicchi F. Universality, limits and predictability of gold-medal performances at the Olympic Games[J]. PloS one, 2012, 7(7): e40335.
    \bibitem{8} Andreff W. Economic development as major determinant of Olympic medal wins: predicting performances of Russian and Chinese teams at Sochi Games[J]. International Journal of Economic Policy in Emerging Economies, 2013, 6(4): 314-340.
